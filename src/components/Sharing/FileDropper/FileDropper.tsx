...
import { FileDropperUtil } from "../../../lib/FileDropper";
...
const FileDropper = (): React.ReactElement => {
  ...
  const onFileInputChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files) {
      const newFilesObject = await FileDropperUtil.getUniqueFiles(
        droppedFiles,
        files
      );
        ...
        <>
          <DroppedFilesWrapper>
            {Object.keys(droppedFiles).map((fileHash) => {
              ...
              const sendProgress = FileStore.filesSendProgress[fileHash];
